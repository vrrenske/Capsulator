---
title: "autocorrelation function-capsulator"
...
spots_perchannel <- spots |>  mutate(ellipse_min = ELLIPSE_MINOR) |> 
  filter(AREA < 1.6) |> gather("Channel", "Intensity_Mean", c("MEAN_INTENSITY_CH1","MEAN_INTENSITY_CH2", "MEAN_INTENSITY_CH3", "ellipse_min")) |> type_convert(guess_integer=TRUE)

spots_perchannel <- spots_perchannel |> select(TRACK_ID, POSITION_X, POSITION_Y, POSITION_T, Intensity_Mean, Channel, FRAME, AREA) |> distinct()
...
ggplot(spots_pT) + geom_line(aes(x=TIME_REL, y= acf_nolag, group=paste(TRACK_ID, Channel), color=Channel, linetype=as.factor(TRACK_ID)), alpha=0.5) + theme_minimal() + geom_smooth(aes(x=TIME_REL, y=acf_nolag,  color=Channel), fill=NA)
...
spots_singlecell <- lapply(all_sub, function(x) get_singlecells(x)) |> 
  bind_rows()|> 
  mutate(ellipse_maj = ELLIPSE_MAJOR) |>
  gather("Channel", "Intensity_Mean", c("MEAN_INTENSITY_CH1","MEAN_INTENSITY_CH2", "MEAN_INTENSITY_CH3", "ellipse_maj")) |>
...
spots_singlecell <- spots_singlecell |> 
  ...
  mutate(ELLIPSE_MAJOR = as.numeric(ELLIPSE_MAJOR),
         ...
         peaks_color = ggpmisc:::find_peaks(acf_percell, span=5, ignore_threshold=0.3),
...
#ggplot(spots_singlecell |> filter(TRACK_ID==1, Leaf==4261), aes(x=POSITION_T, y=ELLIPSE_MAJOR)) + geom_point(aes(color=peaks)) + geom_path()

ggplot(spots_singlecell |> filter(generation>0)) + 
  geom_line(aes(x=TIME_REL/60/60, y=acf_percell, group=paste(Channel,TRACK_ID,Leaf), color=Channel), alpha=0.3) + facet_wrap(~Channel) +
  geom_smooth(aes(x=TIME_REL/60/60, y=acf_percell, group=Channel), fill=NA, color="black", size=2) +
  geom_smooth(aes(x=TIME_REL/60/60, y=acf_percell, color=Channel), fill=NA) +
   scale_color_manual(values=c("MEAN_INTENSITY_CH1" = "#cc183a", "MEAN_INTENSITY_CH2" = "#bcff00", "MEAN_INTENSITY_CH3" = "#2986cc")) -> plot_perChannel_time


spots_pT <- spots_pT |> left_join(spots_singlecell |> ungroup() |> select(FRAME, generation, peaks, TRACK_ID, generation_tot) |> distinct())
